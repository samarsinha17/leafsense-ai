...
export function Profile() {
  ...
  return (
    <section className="mx-auto max-w-5xl px-6 py-16 lg:px-8">
      <h1 className="font-heading text-4xl font-bold">My Profile</h1>
      <Card className="mt-8">
        ...
        <div className="mt-8 grid gap-4 md:grid-cols-3">
          ...
          <div className="rounded-2xl border border-border p-4">
            <p className="text-sm text-muted">Last Login / Update</p>
            <p className="mt-2 font-semibold">{user.updatedAt ? new Date(user.updatedAt).toLocaleString() : "Current session"}</p>
          </div>
          <div className="rounded-2xl border border-border p-4">
            <p className="text-sm text-muted">Login Method</p>
            <p className="mt-2 font-semibold">{user.isVerified ? "Google / Verified account" : "Email password"}</p>
          </div>
        </div>
      </Card>
    </section>
