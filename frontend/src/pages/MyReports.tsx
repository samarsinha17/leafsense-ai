...
export function MyReports() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-6 py-16 lg:px-8">
      <h1 className="font-heading text-4xl font-bold">My Reports</h1>
      <div className="mt-8 grid gap-5">
        {reports.length ? reports.map((report) => (
          <Card key={report.id} className="grid gap-4 md:grid-cols-[1fr_auto]">
            <div>
              ...
              <p className="mt-2 text-sm text-muted">Report ID: {report.report_id}</p>
        ...
        )) : <Card><p className="text-muted">No reports downloaded yet. Download a PDF or CSV from a diagnostic result to see it here.</p></Card>}
      </div>
    </section>
  );
}
